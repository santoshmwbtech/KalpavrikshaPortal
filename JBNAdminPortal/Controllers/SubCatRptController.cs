...
{   
    public class SubCatRptController : Controller
    {
        ...
        public ActionResult Index()
        {
            if (Session["UserID"] != null)
            {
                ...
                ViewBag.SubCategoryList = new SelectList(DAL.GetSubCategoryList(), "ID", "SubCategoryName");
                return View();
            }
            else
            {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SubCatRptList()
        {
            if (Session["UserID"] != null)
            {
                CategoryRpt sub = new CategoryRpt();
                ConsolidatedReport consolidated = DAL.SubCatReport(sub);
        ...
        public ActionResult SearchReport(Search search)
        {
            if (Session["UserID"] != null)
            {
                ...
                return PartialView("SubCatRptList", consolidated);
            }
            else
            {
                return RedirectToAction("Index", "Login");
        ...
        public JsonResult Promotion(ConsolidatedReport consolidatedReport, HttpPostedFileBase postedFile, HttpPostedFileBase notificationImage)
        {
            if (Session["UserID"] != null)
            {
                ...
                string Result = DAL.Promotion(consolidatedReport, Convert.ToInt32(Session["UserID"].ToString()), 2, MailAttachments, ImageURL);
